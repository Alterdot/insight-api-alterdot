...
[![Build Status](https://github.com/Alterdot/insight-api-alterdot/actions/workflows/test_and_release.yml/badge.svg)](https://github.com/Alterdot/insight-api-alterdot/actions/workflows/test_and_release.yml)
[![NPM version](https://img.shields.io/npm/v/insight-api-alterdot.svg)](https://npmjs.org/package/insight-api-alterdot)
...
A Alterdot blockchain REST and WebSocket API Service

This is a backend-only service. If you're looking for the web frontend application, take a look at https://github.com/Alterdot/insight-ui.
...
- [API HTTP Endpoints](#api-http-endpoints)
    ...
    - [Status of the Alterdot Network](#status-of-the-alterdot-network)
...
cd mynode
alterdot-node install insight-api
alterdot-node start  # to also start the service
```
...
To protect the server, insight-api has a built-in query rate limiter. It can be configurable in `alterdot-node.json` with:
...
With all the configuration options available: https://github.com/Alterdot/insight-api-alterdot/blob/master/lib/ratelimiter.js#L10-17
...
### Status of the Alterdot Network
...
`<alterdotAddress>`: new transaction concerning <alterdotAddress> received from network. This event is published in the `<alterdotAddress>` room.
...
- The response for the `/sync` endpoint does not include `startTs` and `endTs` as the sync is no longer relevant as indexes are built in alterdotd.
- The endpoint for `/peer` is no longer relevant connection to alterdotd is via ZMQ.
...
The `/status?q=getTxOutSetInfo` method has also been removed due to the query being very slow and locking alterdotd.
...
- (Medium)[How to setup a Alterdot Instant-Send Transaction using Insight API - The comprehensive way](https://medium.com/@obusco/setup-instant-send-transaction-the-comprehensive-way-a80a8a0572e)

## Contributing

Feel free to dive in! [Open an issue](https://github.com/Alterdot/insight-api/issues/new) or submit PRs.
