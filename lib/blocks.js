...
BlockController.prototype.checkBlockHash = function(req, res, next) {
  ...
  if (hash.length < 64 || !isHexadecimal(hash)) {
    this.node.services.alterdotd.getBlockHeader(parseInt(hash), function(err, info) {
      if (err) {
...
BlockController.prototype.blockHeader = function(req, res, next) {
	...
	self.node.services.alterdotd.getBlockHeader(hash, function(err, info) {
		if (err) {
			return self.common.handleErrors(err, res);
		}
...
BlockController.prototype.blockHeaders = function(req, res, next) {
	...
	if(blockIdentifier.length===64){
		...
		var result = self.node.services.alterdotd.getBlockHeaders(hash, cb, nbOfBlockToFetch);
	}else{
		var height = blockIdentifier;
		var result = self.node.services.alterdotd.getBlockHeaders(height, cb, nbOfBlockToFetch);
	}
...
BlockController.prototype.block = function(req, res, next) {
  ...
  if (blockCached) {
    blockCached.confirmations = self.node.services.alterdotd.height - blockCached.height + 1;
...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix alterdot to give back null instead of null hash
...
BlockController.prototype.blockIndex = function(req, res) {
  ...
  this.node.services.alterdotd.getBlockHeader(parseInt(height), function(err, info) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    res.jsonp({
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.alterdotd.getRawBlock(hash, function(err, blockBuffer) {
      if (err) {
        return next(err);
      }

      var br = new alterdot.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in alterdot-node.
      var header = alterdot.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [alterdot.Transaction().fromBufferReader(br)];

      self.node.services.alterdotd.getBlockHeader(hash, function(err, blockHeader) {
        if (err) {
          return next(err);
        }
        ...
        var confirmations = self.node.services.alterdotd.height - height + 1;
...
BlockController.prototype.list = function(req, res) {
  ...
  self.node.services.alterdotd.getBlockHashesByTimestamp(lte, gte, function(err, hashes) {
    if(err) {
      return self.common.handleErrors(err, res);
    }
...
BlockController.prototype.getPreviousBlock = function(prevHash, cb) {
  ...
  if(prevHash===null) cb(null,null);
      ...
      self.node.getBlock(prevHash, function (err, block) {
          ...
          self.node.services.alterdotd.getBlockHeader(prevHash, function (err, info) {
              if (err) {
...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  ...
  self.getPreviousBlock(prevHash, function(err, info) {
    ...
    if(info===null){
        cb(null, parseFloat("0".toString(10)).toFixed(8));
    }
    else{
	    ...
	    if (nPrevHeight == 0) {
	      // premine
	      nSubsidy = 475000;
      } else if (nPrevHeight <= 375000) {
        nSubsidy = 11;
      } else if (nPrevHeight <= 550000) {
        nSubsidy = 10.5;
      } else if (nPrevHeight <= 625000) {
        nSubsidy = 11;
      } else if (nPrevHeight <= 999999) {
	      nSubsidy = 10.5;
	    } else if (nPrevHeight == 1000000) {
	      nSubsidy = 0.5;
	    } else if (nPrevHeight < 1043800) {
	      nSubsidy = 14;
      } else if (nPrevHeight < 1087600) {
	      nSubsidy = 12;
	    } else {
        nSubsidy = 10;
      }
