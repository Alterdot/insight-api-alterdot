...
describe('Currency', function() {
  ...
  var alterdotCentralData = {
    ...
    exchange_rates: {
      alterdot_usd: 9.4858840414,
      btc_usd: 682.93,
      btc_alterdot: 0.01388998
    }
  };
  ...
  it.skip('will make live request to dash central', function(done) {
    ...
    var res = {
      jsonp: function(response) {
        ...
        should.exist(response.data.alterdot_usd);
        (typeof response.data.alterdot_usd).should.equal('number');
        done();
      }
    };
  ...
  it('will retrieve a fresh value', function(done) {
    var TestCurrencyController = proxyquire('../lib/currency', {
      request: sinon.stub().callsArgWith(1, null, {statusCode: 200}, JSON.stringify(alterdotCentralData))
    });
    var node = {
      log: {
        error: sinon.stub()
    ...
    var res = {
      jsonp: function(response) {
        response.status.should.equal(200);
        should.exist(response.data.alterdot_usd);
        response.data.alterdot_usd.should.equal(9.4858840414);
        done();
      }
    };
    currency.index(req, res);
